...
import { RcN1Controller } from './rc-n1-controller';
...
const main = async () => {
    ...
    boundDimensionsOverlay.hidden = true;   // viewer mode: hide boundary-dimension labels
    ...
    // viewer mode: hide the reference grid (infinite grid plane)
    scene.grid.visible = false;

    // === Gamepad controller: enable gamepad-based 3DGS navigation ===
    // Supports GameSir T4N Lite (Nova Lite) 2.4G with two modes:
    // - Gamepad mode (FPS roaming): left stick move, right stick look, triggers height
    // - Drone mode (American-hand flight): left stick throttle+yaw, right stick pitch+roll
    const gamepadController = new GamepadController(scene.camera, events);
    // Suppress unused variable warning
    void gamepadController;

    // RC-N1 遥控器控制（Electron 桌面版经 preload 注入 window.rcApi 桥接；
    // 浏览器环境无 rcApi 时自动停用，不影响网页版）
    const rcN1Controller = new RcN1Controller(scene.camera, events);
    // Suppress unused variable warning
    void rcN1Controller;

    // Screenshot / video recording / feedback toasts (BACK/SHARE/START keys)
    const gamepadCapture = new GamepadCapture(scene, editorUI.canvas, events);
    // Suppress unused variable warning
    void gamepadCapture;

    // handle load params
    // Single-model semantics: import all load params as one batch so the
    // "clear existing scene" step in file-handler runs only once, not per-file.
    const loadList = url.searchParams.getAll('load');
    const filenameList = url.searchParams.getAll('filename');
    if (loadList.length > 0) {
        const files = loadList.map((value, i) => {
            const decoded = decodeURIComponent(value);
            const filename = i < filenameList.length ?
                decodeURIComponent(filenameList[i]) :
                decoded.split('/').pop();
            return { filename, url: decoded };
        });
