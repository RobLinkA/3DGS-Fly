...
const initFileHandler = (scene: Scene, events: Events, dropTarget: HTMLElement) => {
    ...
    const importFiles = async (files: ImportFile[], animationFrame = false) => {
        ...
        // Single-model semantics: importing a new model replaces the current one.
        // Clear any existing splats first so only one model is ever loaded
        // (covers menu import, drag & drop, empty-state button, and URL loading).
        if (events.invoke('scene.empty') !== true) {
