...
class EditorUI {
    ...
    constructor(events: Events) {
        ...
        const gamepadMenu = new GamepadMenu(events, tooltips);
        ...
        canvasContainer.append(gamepadMenu);

        // === Viewer mode: hide all editing UI, keep only viewing components ===
        // The app is converted to a pure 3DGS viewer with gamepad support.
        // Hidden components are kept in the DOM (for potential re-enable) but
        // made invisible via CSS.
        ...
        menu.hidden = true;             // top menu bar (File/Edit/...)
        toolsContainer.hidden = true;   // selection tools / cursor overlay
        appLabel.hidden = true;

        // view axes container (hidden: pure viewer mode)
        ...
        // === Viewer mode: hide all editing panels ===
